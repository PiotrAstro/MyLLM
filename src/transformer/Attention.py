...
class MultiHeadAttentionFast(torch.nn.Module):
    def __init__(self, input_features: int, output_features: int, num_heads: int, QK_features_n: int, attention_dropout: float, residual_dropout: float):
        assert output_features % num_heads == 0
        super(MultiHeadAttention, self).__init__()
        self.attention_heads = torch.nn.ModuleList([
            AttentionHead(input_features, output_features // num_heads, QK_features_n, attention_dropout, residual_dropout) for _ in range(num_heads)
        ])
...
class AttentionHead(torch.nn.Module):
    def __init__(self, input_features: int, output_features: int, QK_features_n: int, attention_dropout: float, residual_dropout: float):
